...
import com.divya.pojos.Consultation;
import com.divya.pojos.Doctor;
import com.divya.pojos.MedicalCase;
import com.divya.pojos.Patient;

public class HospitalManagement {

	public static final Logger LOGGER = LogManager.getLogger(HospitalManagement.class.getName());// LoggerExample.testLogger(HospitalManagement.class);
...
//private String hospitalLocation;
	...
	public HospitalManagement() {
		...
		// LoggerExample.testLogger(HospitalManagement.class);
		LOGGER.info("IN HospitalManagement");
	 ...
	 * 
	 * public void setHospitalName(String hospitalName) { this.hospitalName =
	 * hospitalName; }
	 * 
	...
	public void addPatient(String name, int age, String patientID, int phonenumber, String medicalHistory,
			String insurance) {
	...
	public void updatePatientMedicalHistory(String patientID, String medicalHistory) {
		...
		if (count) {
			...
			LOGGER.warn("No records found against patientID");
		}
	}
	...
	public void addDoctor(String doctorName, String doctorID, String specialisation, int age, int phoneNumber) {
		...
		if (!checkDoctor(doctorName)) {
			...
			LOGGER.warn("Doctor already exists");
	...
	private boolean checkDoctorID(String doctorID) {
		...
		LOGGER.warn("Doctor does not exist");
		return false;

	}

	public void addMedicalCase(String caseID, String patientID, String doctorID, String caseDescription,
			String caseStartDate, String caseEndDate) {
		...
		if (checkPatientID(patientID) && checkDoctorID(doctorID)) {
			...
			LOGGER.info(medicalCase);
		} else
			// System.out.println("Patient ID or Doctor ID does not exist");
			LOGGER.warn("Patient ID or Doctor ID does not exist");
	}
	...
	public Patient getPatientByID(String patientID) {
		...
		for (int i = 0; i < patients.size(); i++) {
			if (patients.get(i).getPatientID().equalsIgnoreCase(patientID)) {
				...
				LOGGER.info(patients.get(i).toString());
				return patients.get(i);
			}
		}
		// System.out.println("Given Patient ID doesn't exist");
		LOGGER.warn("Given Patient ID doesn't exist");
		return null;
	}
	...
	public boolean getPatientByName(String patientName) {
		...
		LOGGER.warn("Given Patient name doesn't exist");
		return false;
	}
	...
	public Doctor getDoctorByID(String doctorID) {
		...
		LOGGER.warn("Given Doctor ID doesn't exist");
		// return false;
		return null;
	}
	...
	public boolean getDoctorByName(String doctorName) {
		for (int i = 0; i < doctors.size(); i++) {
			if (doctors.get(i).getDoctorName().equalsIgnoreCase(doctorName)) {
				// System.out.println(doctors.get(i).toString());
				LOGGER.debug(doctors.get(i).toString());
				return true;
			}

		}
		// System.out.println("Given Doctor Name doesn't exist");
		LOGGER.warn("Given Doctor Name doesn't exist");
		return false;
	}
	...
	public MedicalCase getMedicalCaseByID(String caseID) {
		...
		LOGGER.warn("Given Case ID doesn't exist");
		return null;
	}
	...
	public boolean checkCaseID(String caseID) {
		...
		LOGGER.warn("Case ID doesn't exist");
		return false;
	}
	...
	public void addConsultation(String consultationID, String caseID, String appointmentTime, String followUpDate) {
		...
		if (checkCaseID(caseID)) {
			...
			LOGGER.warn("Patient doesn't exist");
	}
	...
	public Consultation getConsultation(String consultationID) {
		...
		LOGGER.warn("Case ID doesn't exist to add consultation");
		return null;
	}
	...
	public boolean checkConsultationID(String consultationID) {
		...
		LOGGER.warn("Consultation ID doesn't exist");
		return false;
	}
